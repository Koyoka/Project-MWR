...
-- -----------------------------------------------------
-- Table `MWRData`.`MWDestroyMCParamsLog`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `MWRData`.`MWDestroyMCParamsLog` (
  `MCCode` VARCHAR(45) NOT NULL COMMENT '设备编号',
  `RunDate` DATETIME NOT NULL COMMENT '运行时间',
  `WSCode` VARCHAR(20) NULL,
  `DisiNum` INT NULL COMMENT '消毒批次',
  `Pressure` FLOAT NULL COMMENT '压力',
  `InTemperature` FLOAT NULL COMMENT '内部温度',
  `ExTemperature` FLOAT NULL COMMENT '外部温度',
  `EC1` FLOAT NULL COMMENT 'Electric current 电流 电机1',
  `EC2` FLOAT NULL COMMENT '电流 电机2',
  `WordStatus` VARCHAR(45) NULL,
  PRIMARY KEY (`MCCode`, `RunDate`))
ENGINE = InnoDB
COMMENT = '处置机器处理参数日志';


-- -----------------------------------------------------
-- Table `MWRData`.`MWDestroyMCDetail`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `MWRData`.`MWDestroyMCDetail` (
  `MCDetailId` INT NOT NULL,
  `DisiNum` INT NULL COMMENT '批次号',
  `TxnDetailId` INT NULL,
  `WSCode` VARCHAR(20) NULL,
  `CrateCode` VARCHAR(45) NULL,
  `DestWeight` DECIMAL(10,2) NULL,
  `EntryDate` DATETIME NULL,
  `Status` VARCHAR(2) NULL,
  PRIMARY KEY (`MCDetailId`))
ENGINE = InnoDB
COMMENT = '机器处理废料的批次详情';
