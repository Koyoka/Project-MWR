...
{
    public partial class FrmInitDWSConfig : Form
    {
        private const string ClassName = "YRKJ.MWR.WinBase.WinAppBase.BaseForm.FrmInitDWSConfig";
        private FormMng _frmMng = null;
        public delegate bool DelegateInitValid(AppConfig cfg);
        public DelegateInitValid OnInitValid = null;

        public FrmInitDWSConfig()
        {
            InitializeComponent();
        ...
        private void c_btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cfg.ServiceRoot = c_txtModbusIp.Text;
                if (OnInitValid != null)
                {
                    if (!OnInitValid(cfg))
                    {
                ...
                cfg.ModbusIp = c_txtModbusIp.Text.Trim();
                cfg.ModbusPort = c_txtModbusPort.Text.Trim();
                int v = 0;
                if (!ComLib.ComFn.StringToIntUnSafe(cfg.ModbusPort, ref v))
                {
                    MsgBox.Error(LngRes.MSG_ModbusPortError);
                    return;
                }
                if (string.IsNullOrEmpty(cfg.ModbusIp))
                {
                    MsgBox.Error(LngRes.MSG_ModbusIPError);
                    return;
                }
                #endregion
        ...
        private class LngRes
        {
            ...
            public const string MSG_ModbusPortError = "设备连接端口错误";
            public const string MSG_ModbusIPError = "设备连接IP不能为空";
