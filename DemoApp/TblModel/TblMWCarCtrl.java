...
public class TblMWCarCtrl extends BaseDataCtrl {

    public static List<TblMWCar> QueryPage(DataCtrlInfo dcf,SqlQueryMng sqm,int page,int pageSize){
        List<TblMWCar> itemList = null;
        try{
            sqm.setQueryTableName(TblMWCar.getFormatTableName());
            String sql = sqm.getPageSql(page, pageSize);
            SqlCommonFn.DebugLog(sql);
            itemList = SqlDBMng.getInstance().query(sql, TblMWCar.class,sqm.getParamsArray());
            if(itemList.size() != 0){
    ...
    public static List<TblMWCar> QueryMore(SqlWhere sw) throws Exception {
        SqlQueryMng sqm = new SqlQueryMng();
        sqm.Condition.Where.AddWhere(sw);
        return QueryMore(sqm);
    }

    public static List<TblMWCar> QueryMore(DataCtrlInfo dcf,SqlWhere sw) {
    ...
    public static List<TblMWCar> QueryMore(SqlQueryMng sqm) throws Exception {
        sqm.setQueryTableName(TblMWCar.getFormatTableName());
        String sql = sqm.getSql();
        SqlCommonFn.DebugLog(sql);//System.out.println(sql);
        return SqlDBMng.getInstance().query(sql, TblMWCar.class,sqm.getParamsArray());
    }

    public static List<TblMWCar> QueryMore(DataCtrlInfo dcf,SqlQueryMng sqm){
        List<TblMWCar> itemList = null;
        try {
    ...
    public static TblMWCar QueryOne(SqlQueryMng sqm) throws Exception {
        List<TblMWCar> itemList = QueryMore(sqm);
        if(itemList.size() > 0){
    ...
    public static TblMWCar QueryOne(DataCtrlInfo dcf,SqlQueryMng sqm) {
        TblMWCar item = null;
        try {
            item = QueryOne(sqm);
            dcf.set(true, "");
    ...
    public static TblMWCar QueryOne(SqlWhere sw) throws Exception {
        SqlQueryMng sqm = new SqlQueryMng();
        sqm.Condition.Where.AddWhere(sw);
        return QueryOne(sqm);
    }

    public static TblMWCar QueryOne(DataCtrlInfo dcf,SqlWhere sw) {
        TblMWCar item = null;
        try {
            item = QueryOne(sw);
            dcf.set(true, "");
    ...
    public static int Insert(DataCtrlInfo dcf,TblMWCar item) {
        return Insert(dcf,
                item.CarCode,
                item.Desc
                );
    ...
    public static int Update(DataCtrlInfo dcf,TblMWCar item,SqlWhere sw) {
