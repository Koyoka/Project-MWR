...
public class TblMWSynclogCtrl extends BaseDataCtrl {

    public static List<TblMWSynclog> QueryPage(DataCtrlInfo dcf,SqlQueryMng sqm,int page,int pageSize){
        List<TblMWSynclog> itemList = null;
        try{
            sqm.setQueryTableName(TblMWSynclog.getFormatTableName());
            String sql = sqm.getPageSql(page, pageSize);
            SqlCommonFn.DebugLog(sql);
            itemList = SqlDBMng.getInstance().query(sql, TblMWSynclog.class,sqm.getParamsArray());
            if(itemList.size() != 0){
    ...
    public static List<TblMWSynclog> QueryMore(SqlWhere sw) throws Exception {
        SqlQueryMng sqm = new SqlQueryMng();
        sqm.Condition.Where.AddWhere(sw);
        return QueryMore(sqm);
    }

    public static List<TblMWSynclog> QueryMore(DataCtrlInfo dcf,SqlWhere sw) {
    ...
    public static List<TblMWSynclog> QueryMore(SqlQueryMng sqm) throws Exception {
        sqm.setQueryTableName(TblMWSynclog.getFormatTableName());
        String sql = sqm.getSql();
        SqlCommonFn.DebugLog(sql);//System.out.println(sql);
        return SqlDBMng.getInstance().query(sql, TblMWSynclog.class,sqm.getParamsArray());
    }

    public static List<TblMWSynclog> QueryMore(DataCtrlInfo dcf,SqlQueryMng sqm){
        List<TblMWSynclog> itemList = null;
        try {
    ...
    public static TblMWSynclog QueryOne(SqlQueryMng sqm) throws Exception {
        List<TblMWSynclog> itemList = QueryMore(sqm);
        if(itemList.size() > 0){
    ...
    public static TblMWSynclog QueryOne(DataCtrlInfo dcf,SqlQueryMng sqm) {
    ...
    public static TblMWSynclog QueryOne(SqlWhere sw) throws Exception {
        SqlQueryMng sqm = new SqlQueryMng();
        sqm.Condition.Where.AddWhere(sw);
        return QueryOne(sqm);
    }

    public static TblMWSynclog QueryOne(DataCtrlInfo dcf,SqlWhere sw) {
        TblMWSynclog item = null;
        try {
            item = QueryOne(sw);
            dcf.set(true, "");
    ...
    public static int Insert(DataCtrlInfo dcf,TblMWSynclog item) {
        return Insert(dcf,
                item.SyncDateTime,
                item.Status,
                item.Remark
    ...
    public static int Update(DataCtrlInfo dcf,TblMWSynclog item,SqlWhere sw) {
